...
./klee --output-dir=$OUTDIR/klee-pmdk-btree-static --search=nvm --nvm-heuristic-type=static \
  ...
  000_Buggy.bc --sym-pmem-delay PMEM 8388608 PMEM --sym-arg 2 &
...
# 3. BFS
./klee --output-dir=$OUTDIR/klee-pmdk-btree-bfs --search=bfs \
  --custom-checkers=false --max-time=86400 --write-errors-only=true --max-memory=10000  \
  --disable-verify=true --libc=uclibc --link-known-lib=libpmemobj --link-known-lib=libpmem \
  --posix-runtime --env-file=pmdk.env \
  000_Buggy.bc --sym-pmem-delay PMEM 8388608 PMEM --sym-arg 2 &
...
# 4. DFS
./klee --output-dir=$OUTDIR/klee-pmdk-btree-dfs --search=dfs \
  --custom-checkers=false --max-time=86400 --write-errors-only=true --max-memory=10000  \
  --disable-verify=true --libc=uclibc --link-known-lib=libpmemobj --link-known-lib=libpmem \
  --posix-runtime --env-file=pmdk.env \
  000_Buggy.bc --sym-pmem-delay PMEM 8388608 PMEM --sym-arg 2 &
...
# 5. Coverage new 
./klee --output-dir=$OUTDIR/klee-pmdk-btree-covnew --search=nurs:covnew \
  --custom-checkers=false --max-time=86400 --write-errors-only=true --max-memory=10000  \
  --disable-verify=true --libc=uclibc --link-known-lib=libpmemobj --link-known-lib=libpmem \
  --posix-runtime --env-file=pmdk.env \
  000_Buggy.bc --sym-pmem-delay PMEM 8388608 PMEM --sym-arg 2 &
