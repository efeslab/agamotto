...
#define NVM_USID_EQ(G1, G2) ((G1).d1 == (G2).d1 && (G1).d2 == (G2).d2)
        ...
        )
    {
        ...
        if (ptr == NULL)
            return;
        nvm_usid u1 = *((nvm_usid*)ptr);
        nvm_usid u2 = def->usid;
        if (!nvm_usid_eq(u1, u2))
        {
            printf("nvm_usid ERROR: %lu, %lu\tu2:%lu, %lu\n",
                u1.d1, u1.d2, u2.d1, u2.d2);
            if (u1.d1 == u1.d2 && u1.d1 == 0) {
              nvms_assert_fail("impossible nvm_usid");
            }
            nvm_upgrade(ptr, def);
        }
    }
