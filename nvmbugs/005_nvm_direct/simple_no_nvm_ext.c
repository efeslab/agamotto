...
main(int argc, char** argv)
{
    ...
    printf("calling nvm_attach_region\n");
    nvm_desc desc = nvm_attach_region(0, fname, attch);
    printf("done with nvm_attach_region\n");
    ...
    printf("Fuck it, let's see how far we get eh?\n");
    //exit(0);

    /* If extents not added then add them. */
		printf("querying extents\n");
    nvm_extent_stat xstat[4];
    nvm_query_extents(desc, 1, 4, xstat);
		printf("done with querying extents\n");
    if (xstat[0].psize == 0)
    {
				printf("i guess, trying to add an application managed extent now\n");
    ...
    {
				printf("i guess, xstat[0].psize was already non-zero?\n");
    ...
    printf("maxtx:%d undo:%d limit:%d\n", cfg.txn_slots, cfg.undo_blocks,
		...
		printf("DRIVER: finna boutta call nvm_heap_test\n");
    nvm_heap_test(desc, nvm_heap_get(&rs->heap));
		printf("DRIVER: after nvm_heap_test\n");
