        ...
        )
{
    ...
    uint64_t clsz = td->params.cache_line; // cache line size
    //stolerbs: cacheline-aligned of ptr
    uint64_t cl = (uint64_t)ptr & ~(clsz - 1); // cache line address
    uint64_t cnt = ((uint64_t)ptr + bytes - cl + clsz - 1) / clsz;
    ...
    //stolerbs: just call nvms_flush instead
    while(cnt--)
    {
      nvms_flush(cl);
      cl += clsz;
    ...
    /* while (cnt--) */
    /* { */
    /*     /1* get the primary and secondary entries in the cache *1/ */
    /*     uint64_t *pe = fc + index; */
    /*     uint64_t *se = fc + (index ^ td->flush_cache_xor); */
    ...
    /*                 if (*se != cl) // done if already in secondary */
    /*                 { */
    /*                     /1* Both slots full so flush the primary and use it. *1/ */
    /*                     nvms_flush(*pe); */
    /*                     td->flush_cnt++; */
    /*                     *pe = cl; */
    /*                 } */
    ...
    /*     /1* next slot. May wrap to slot zero *1/ */
    /*     if (++index >= td->params.flush_cache_sz) */
    /*         index = 0; */
    /*     // stolerbs: BUG: wtf, doesn't cl need to be incremented?? */
    /*     //PATCH: */
    /*     cl += clsz; */
    /* } */
}
        ...
        )
{
    ...
    //stolerbs: again, just call nvms_flush
    nvms_flush(cl);
    /* /1* get the primary and secondary entries in the cache *1/ */
    /* uint64_t *pe = fc + index; */
    /* uint64_t *se = fc + (index ^ td->flush_cache_xor); */
...
void nvm_persist()
{
    ...
    if (fc != 0)
    {
        ...
        {
            //stolerbs: fc should never be populated
            assert((*fc) == 0);
            if (*fc)
            {
